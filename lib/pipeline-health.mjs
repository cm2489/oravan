 ...
 *   `…; kept tonight: 41 article(s) on 23 bill(s); 3 stored article(s) dropped
 *    on tonight's gate verdict; 0 gate reply(ies) with no usable answer`
 ...
 *   unansweredGates:number|null} | null}
 */
export function parseCoverageDone(log) {
  const m = lastMatch(
    logLines(log),
    /^DONE: (\d+)\/(\d+) bills with coverage, (\d+) articles total(?: \(\+(\d+) unprocessed bills carried forward\))?(?:; kept tonight: (\d+) article\(s\) on (\d+) bill\(s\)(?:; (\d+) stored article\(s\) dropped on tonight's gate verdict)?(?:; (\d+) gate reply\(ies\) with no usable answer)?)?/
  );
  if (!m) return null;
  return {
...
/**
 * A coverage night checked this many bills and kept NOTHING before it is a ⛔.
 * The gate historically keeps 7.5–11% of candidates and a normal night keeps
 * articles on dozens of bills, so zero across 100+ checked bills is a dead
 * source or a dead gate, not a quiet news day.
 */
...
export function estimateDaySpend({ decodes = 0, t3Batched = 0, t3Runs = 0, pregen = null } = {}) {
  ...
  const assumptions = {
    ...
    t3PromptOverheadTokens: 200,
    t3TokensPerHeadline: 150,
...
export function alarms(report) {
  ...
  if (run && typeof run.keptTonight === 'number' && run.keptTonight === 0 && run.checked >= COVERAGE_KEPT_ZERO_MIN_CHECKED) {
    out.push({
      ...
      text: `the nightly coverage sync checked ${run.checked} bills and kept no article on any of them — the news source or the relevance gate is not working (stored coverage was carried over untouched)`,
    });
  }
...
function formatCoverageRun(run) {
  if (!run) return 'not found in the log';
  const tonight =
    typeof run.keptTonight === 'number'
      ? `kept tonight ${run.keptTonight} on ${run.billsKeptTonight} bill(s) · ${n(run.droppedOnVerdict)} dropped on the gate's verdict · ${n(
          run.unansweredGates
        )} unusable gate replies`
      : 'kept tonight not in this log (older format)';
  return `${run.checked} checked · ${tonight} · ${run.withCoverage} with coverage after the merge`;
}
...
export function formatHealthSection(report) {
  ...
  const lines = [
    ...
    row('coverage run', formatCoverageRun(report?.coverageRun)),
...
export function formatHealthIssueBody(report) {
  return [
    ...
    '**A dated comment appears below only when a ⛔ condition holds** — the nightly did ' +
      ...
      'the pipeline kept running, the floor-signals stamp passed its alarm, the coverage ' +
      "sync's 30-day pass shifted its outlet-lean mix against the whole-life pass, or a " +
      'coverage night kept nothing at all. A quiet day rewrites this body and says nothing else.',
  ].join('\n');
}
