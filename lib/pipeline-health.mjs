 ...
 * @returns {{withCoverage:number, checked:number, articles:number, carriedForward:number} | null}
 */
export function parseCoverageDone(log) {
  const m = lastMatch(
    logLines(log),
    /^DONE: (\d+)\/(\d+) bills with coverage, (\d+) articles total(?: \(\+(\d+) unprocessed bills carried forward\))?/
  );
  if (!m) return null;
  return {
 ...
 * CONJUNCTION, not an age: lastSync older than two days while lastRun is
 * inside 48h means the pipeline ran and the cursor did not move. lastSync
 * alone being old on a pipeline that has also stopped running is a different
 * failure (nothing is running at all) and must not wear the same word.
 *
 * @param {{lastSync?: string, lastRun?: string}} state
 * @param {{now?: number}} [opts]
 */
export function cursorHealth(state, { now = Date.now() } = {}) {
...
export function alarms(report) {
  ...
  if (!nightly) {
    out.push({ code: 'nightly-missing', text: 'no nightly bill sync run in the last 24h' });
  ...
  if (report?.cursor?.frozen) {
    out.push({
      code: 'cursor-frozen',
      text: `the corpus cursor has not moved in ${Math.round(report.cursor.lastSyncAgeDays)}d while the pipeline kept running — the backlog scan is not making progress`,
    });
  }
...
export function formatHealthSection(report) {
  ...
  const lines = [
    ...
    '',
    '```',
    row('nightly', nightly ? `${nightly.conclusion} in ${n(nightly.durationMin, 'm')}` : 'no run in 24h'),
    row(
      'sync counters',
      sync
    ...
    row(
      'pregen',
      `${n(report?.pregen?.alreadyCached)} already cached · ${n(report?.pregen?.cached)} written · ${n(
        report?.pregen?.failed
      )} failed · ${n(report?.upstashCacheFailures)} cache-write failures`
    ),
...
export function formatHealthIssueBody(report) {
  return [
    ...
    '**A dated comment appears below only when a ⛔ condition holds** — the nightly did ' +
      ...
      'the pipeline kept running, or the floor-signals stamp passed its alarm. A quiet day ' +
      'rewrites this body and says nothing else.',
  ].join('\n');
}
