 ...
 *     admitted but carries NO lean: it can never make a set look balanced,
 *     and the one-sided rule is still asked only of rated outlets.
 ...
 *     "outlets": {
 *       "example.com": { "approved_on": "YYYY-MM-DD", "note": "why" }
 *     }
 *   }
 *
 ...
 * @returns {{ domains: Set<string>, problems: string[] }}
...
export function parsePressAllowlist(raw) {
  if (raw === null || raw === undefined) return { domains: new Set(), problems: [] };
  const problems = [];
  if (typeof raw !== 'object' || Array.isArray(raw)) {
    return { domains: new Set(), problems: ['root must be an object with an "outlets" map'] };
  }
  const outlets = /** @type {any} */ (raw).outlets;
  if (typeof outlets !== 'object' || outlets === null || Array.isArray(outlets)) {
    return { domains: new Set(), problems: ['"outlets" must be an object keyed by bare outlet domain'] };
  }
  ...
  return problems.length ? { domains: new Set(), problems } : { domains, problems };
}
...
export function pressOutletPolicy({ ratings, allowlist } = {}) {
  ...
  const { domains: allowed, problems } = parsePressAllowlist(allowlist);
