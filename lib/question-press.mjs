 ...
 *  no query-length limit. What is known, all from the build machine with this
 *  project's honest User-Agent (`GDELT_LENGTH_EVIDENCE` holds it as data):
 *
 ...
 *
 *    ANSWERED with an article list — at 54 characters, in a different shape
 *    (`"war powers" (congress OR senate) domainis:foxnews.com`).
 *
 *    NOT MEASURED: the shape this file now sends. The 2026-09-26 pass had a
 *    budget of six live requests in the exact shape `buildGdeltQuery` +
 *    `gdeltUrl` produce, at least 60 s apart. None drew a verdict on length:
 ...
 *    `httpsFetch`, which is why the collector no longer uses it) and the four
 *    others drew HTTP 429 "Please limit requests to one every 5 seconds"
 *    after 4, 10, 30 and 76 minutes of silence from that IP.
 *    `GDELT_LENGTH_EVIDENCE.inconclusive` lists all six.
 *
 *  So 100 is a CHOICE, not a measurement — `capVerified: false` says so, and
 *  the run log repeats it until the cap is verified. It is the smallest round
 *  cap that keeps every live question's phrases (the longest, "continuing
 *  appropriations and extensions act", makes a 99-character query), and it is
 *  under a third of the shortest length GDELT has refused. If GDELT refuses
 *  at this length anyway, the cost is bounded and visible: that one term is
 *  left out with GDELT's sentence in a ::warning:: (one request a day), and
 *  three refusals in a row open the circuit (scripts/gdelt-intake.mjs
 *  REFUSAL_CIRCUIT) — never the old build's loop of halving and re-sending.
 *  Every run prints the longest length GDELT answered; once a runner records
 *  an answer at or above the cap, add it to `answered` and set `capVerified`
 *  to true. A phrase whose query would need more than the cap is dropped
 *  before it is sent (`questionTerms`).
 */
export const GDELT_MAX_QUERY_CHARS = 100;

/** Every query-length measurement behind `GDELT_MAX_QUERY_CHARS`, as data, so
 *  a test holds the cap and its claim to them. `answered` = GDELT returned an
 *  article list (JSON); `refused` = GDELT returned "Your query was too short or
 *  too long."; `inconclusive` = requests in the current shape that drew no
 *  verdict on length (a 429, or no answer at all). */
