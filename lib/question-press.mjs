 ...
 *   malformed — JSON that does not parse (a truncated body) or has the wrong
 *               shape, or markup (an HTML error page from GDELT's front end or
 *               a proxy). A passing failure of the transport, not a verdict on
 *               the search, so the question is retried as a whole.
 *
...
/**
 * Write ONLY WHAT A RUN RECORDED.
 *
 *   - A run that recorded no check writes NOTHING: not a link that aged out,
 ...
 *
 * @param {{ previous: any, next: any, recorded: number }} input
 *   `recorded` = how many questions this run checked successfully.
 */
export function shouldWrite({ previous, next, recorded }) {
 ...
 * of the newest check it holds — because a run that records nothing writes
 * nothing (`shouldWrite`), so that day is exactly the last day any run
 * recorded a check. Reads each entry's `checkedOn` and nothing else: no count,
 * no outlet, no link reaches the digest from here.
 *
 * @param {{
 *   data: any,
 ...
 *
 * WARNS on: a question that is no longer live (the next write drops it); a
 ...
 *
 * TWO CHECKS READ OTHER FILES, and they are the file's coupling to the rest of
 * data/: every question id must exist in data/moments.json, and every outlet
 * must carry the same AllSides lean in data/media-bias.json. So a change to
 * either file that deletes a question id, or un-rates or re-rates an outlet
 * this file holds, FAILS this gate — in that change's own PR CI (which reads
 * the PR's moments/bias against the committed evidence), in ci.yml on main,
 * and in the nightly's pre-commit scripts/verify-sync.mjs — until this file is
 * rewritten. The collector rewrites it (re-judging every carried outlet
 * against the current table and keeping live questions only) on its next run
 * that RECORDS a check (`shouldWrite`); a run GDELT refuses throughout writes
 * nothing, so during a GDELT outage the failure stands until GDELT answers
 * again — unless the change that caused it also edits this file to match
 * (removes the question's entry, or the outlet with its counts). Retiring a
 * question (a status other than live) is only a warning, and adding a
