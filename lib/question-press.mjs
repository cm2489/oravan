 ...
 *  every query of the collector's full shape that it answered at all: 1,002
 *  characters (the funding question, all 25 right-rated domains), 976 (the
 *  same without its two-letter words), 734 (Iran, all 20 center domains), 583
 *  (Iran, 14), 436 (Iran, 8) and 333 ("war powers" + the context words + 8
 *  center domains). Every query this file built before that was 637–1,120
 *  characters, so none of them could ever have run. A 54-character query
 *  ("war powers" (congress OR senate) domainis:foxnews.com) WAS answered
 *  with an article list; the probes between (331, 289, 265 and 225
 *  characters) drew only 429s and connection timeouts from that machine.
 *
 *  So the real ceiling is UNMEASURED between 54 and 333 — and it may be a
 *  count of OR'd items rather than characters; the probes cannot tell the
 *  two apart. This value is the largest length not yet seen refused, not a
 *  known-good one. The collector corrects it downward on its own: a "too
 *  short or too long" answer halves the refused group and lowers the run's
 *  limit (scripts/gdelt-intake.mjs), and the run log prints the longest length
 *  GDELT answered and the shortest it refused — the number to set here once a
 *  runner has measured it. A question whose terms and the context words
 *  alone do not fit beside one domain is not searched at all (the funding
 *  question, at 355 characters, today). See `domainChunks`. */
export const GDELT_MAX_QUERY_CHARS = 330;
