 ...
 *     an owner allowlist, if one is ever trialled, plugs into
 *     `eligibleDomainsByLean` and the gate in `verifyQuestionPress` together.
 *
 ...
 *   The terms are the owner-curated `aliases.en` plus each vehicle's bill
 *   names (`press_names`, `short_title`). The AI-generated `news_query`
 *   phrases are deliberately NOT used: "fiscal 2027" and "troop levels" match
 *   every appropriations story, and the owner's aliases are the vocabulary a
 *   person answers for. The exact terms used are stored beside the evidence,
 *   so anyone can re-run the search.
 *
 ...
 */
import { RATED_LEANS, dayKey, daysBetween, leanOf, normalizeDomain } from './conversation.mjs';
...
/** Strip the fragment and utm_* tracking parameters; keep everything else.
...
export function parseArtList(body) {
  ...
  try {
    ...
    return { ok: false, error: `not JSON: ${String(body ?? '').replace(/\s+/g, ' ').trim().slice(0, 160)}` };
  }
