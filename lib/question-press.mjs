 ...
 *    `httpsFetch`, which is why the collector no longer uses it) and the rest
 *    drew HTTP 429 "Please limit requests to one every 5 seconds" after 4, 10
 *    and 30 minutes of silence from that IP. `GDELT_LENGTH_EVIDENCE
 *    .inconclusive` lists them.
 *
