 ...
 * scripts/verify-sync.mjs, where it only ever warns (this optional file never
 * fails the nightly — `verifyQuestionPress`). Nothing in here touches the
 * network or the disk.
 *
 * ---- WHY THIS EXISTS -------------------------------------------------------
 ...
 * exception is a search GDELT REFUSES: an answer in plain text instead of
 * results ("Your query was too short or too long.", or any other sentence
 * GDELT writes instead of running the search — `parseArtList`). That is
 * treated as GDELT's verdict on that one search: the term is left out of this
 * run's search with GDELT's answer quoted verbatim in a ::warning::, the
 * question still updates from its other searches, and the stored `terms` are
 * exactly the ones GDELT answered, so anyone can re-run the search. Dropping a
 * term drops it for every lean. If a plain-text answer is ever a passing
 * error rather than a verdict, the cost is that term missing from one day's
 * search, said in the warning and in the stored terms — never a question
 * frozen for good by a sentence the parser did not know, which is what the
 * first version of this rule did (it knew two sentences and read every other
 * one as a malformed body).
 *
 * A REFUSED LATER PAGE IS NOT A REFUSED SEARCH. When page 1 of a term came
 * back full (250 records) and GDELT refuses the page after it, GDELT has
 * already ANSWERED that search. The term stays in the stored `terms`, the
 * evidence its answered pages returned stays in the file, and the term is
 * marked TRUNCATED with the same loud warning as a term whose paging ran out:
 * the oldest part of its window was not seen, for every lean alike. Dropping
 * the term there would leave links in the file that came from a search the
 * stored `terms` no longer name — the one thing "the stored terms are exactly
 * the ones GDELT answered" rules out.
 *
...
/** The longest query this pipeline will ever SEND: 99 characters, exactly the
 *  longest query GDELT has been seen to ANSWER in the shape this file sends.
 *  GDELT's DOC 2.0 docs name no query-length limit. What is known
 *  (`GDELT_LENGTH_EVIDENCE` holds it as data; every request below was sent
 *  from the build machine with this project's honest User-Agent, except the
 *  07:36 answer, which was reported to this file by its length, shape and
 *  answer):
 *
 *    ANSWERED in the current shape — at 99 characters, 2026-09-26T07:36:12Z:
 *    `"continuing appropriations and extensions act" (congress OR senate OR
 *    representatives OR lawmakers)`, the exact `buildGdeltQuery` +
 *    `gdeltUrl` request, drew HTTP 200 and the body `{}` (GDELT's no-match
 *    answer: the search was RUN, not refused). That is the live questions'
 *    longest query: the query string recorded for it is the only
 *    99-character query the live questions produce.
 *
 *    ANSWERED with an article list — at 54 characters, in an older shape
 *    (`"war powers" (congress OR senate) domainis:foxnews.com`).
 *
 *    REFUSED — HTTP 200 and the plain-text sentence "Your query was too short
 *    or too long." — at 333, 436, 583, 734, 976 and 1,002 characters: every
 *    query of the old shape (a phrase list, nine context words and 8–25
 *    `domainis:` terms) that GDELT answered at all.
 *
 *    INCONCLUSIVE — seven more requests in the current shape drew no verdict:
 *    two died in connection setup (GDELT's TLS handshake measured 9.2–9.7 s,
 *    and Node's fetch gives up at 10 s — see scripts/gdelt-intake.mjs
 *    `httpsFetch`, which is why the collector no longer uses it) and five drew
 *    HTTP 429 "Please limit requests to one every 5 seconds".
 *
 *  WHY 99 AND NOT A ROUND NUMBER ABOVE IT. Nothing between 100 and 332
 *  characters has been measured, so any cap above 99 would be a guess, and
 *  the only thing a guess could buy is a phrase no live question has (their
 *  longest query is 99). A cap below 99 would drop the funding question's
 *  "continuing appropriations and extensions act" for no measured reason. So
 *  the cap is the evidence, with no margin either way, and a test holds it
 *  there (`GDELT_VERIFIED_QUERY_CHARS`). ASSUMED, not measured: that GDELT's
 *  length rule is a threshold, so a shorter query in the same shape is not
 *  refused for length either.
 *
 *  A phrase whose query would need more than the cap is dropped before it is
 *  sent (`questionTerms`), and the run log names it. To raise the cap, measure
 *  first: run the collector locally with GDELT_MAX_QUERY_CHARS and
 *  GDELT_FORCE=1 (scripts/gdelt-intake.mjs `limitsFrom`); a run that gets an
 *  answer longer than any on record prints a ::notice:: saying so; record it
 *  in `answered`, then raise this. If GDELT ever refuses at or under the cap,
 *  the cost stays bounded and visible: that one term is left out with GDELT's
 *  sentence in a ::warning::, and three refusals in a row open the circuit
 *  (scripts/gdelt-intake.mjs REFUSAL_CIRCUIT) — never the old build's loop of
 *  halving and re-sending.
 */
export const GDELT_MAX_QUERY_CHARS = 99;

/** Every query-length measurement behind `GDELT_MAX_QUERY_CHARS`, as data, so
 *  a test holds the cap to them. `answered` = GDELT ran the search (a JSON
 *  article list, or `{}` for no matches) — `shape: 'current'` marks a query in
 *  exactly the shape `buildGdeltQuery` builds, the only kind that can verify
 *  the cap; `refused` = GDELT returned "Your query was too short or too long.";
 *  `inconclusive` = requests in the current shape that drew no verdict on
 *  length (a 429, or no answer at all). */
export const GDELT_LENGTH_EVIDENCE = Object.freeze({
  inconclusive: Object.freeze([
    { chars: 65, at: '2026-09-26T04:02:21Z', query: '"war powers" (congress OR senate OR representatives OR lawmakers)', result: 'no answer: connection setup passed 10 s (UND_ERR_CONNECT_TIMEOUT)' },
    { chars: 99, at: '2026-09-26T04:03:47Z', query: '"continuing appropriations and extensions act" (congress OR senate OR representatives OR lawmakers)', result: 'no answer: connection setup passed 10 s (UND_ERR_CONNECT_TIMEOUT)' },
    { chars: 99, at: '2026-09-26T04:08:30Z', query: '"continuing appropriations and extensions act" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 429' },
    { chars: 65, at: '2026-09-26T04:18:52Z', query: '"war powers" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 429' },
    { chars: 99, at: '2026-09-26T04:49:33Z', query: '"continuing appropriations and extensions act" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 429' },
    { chars: 99, at: '2026-09-26T06:06:27Z', query: '"continuing appropriations and extensions act" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 429' },
    { chars: 62, at: '2026-09-26T07:44:12Z', query: '"cr bill" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 429 (8 minutes after the 07:36 answer)' },
  ]),
  answered: Object.freeze([
    { chars: 54, on: '2026-09-26', shape: 'domainis (retired)', query: '"war powers" (congress OR senate) domainis:foxnews.com' },
    { chars: 99, at: '2026-09-26T07:36:12Z', shape: 'current', query: '"continuing appropriations and extensions act" (congress OR senate OR representatives OR lawmakers)', result: 'HTTP 200, body {} — no matches: the search was run, not refused' },
  ]),
...
/** The longest query GDELT has answered in the current shape — what
 *  `GDELT_MAX_QUERY_CHARS` may not exceed (a test holds it there). */
export const GDELT_VERIFIED_QUERY_CHARS = Math.max(
  0,
  ...GDELT_LENGTH_EVIDENCE.answered.filter((a) => a.shape === 'current').map((a) => a.chars)
);
...
/** GDELT's plain-text answer to a query too long (or short) for it —
 *  measured 2026-09-26 at 333 to 1,002 characters. Only this sentence is
 *  evidence about the length cap, so the run log counts it apart; every other
 *  plain-text answer is still a refusal of that one search (`parseArtList`). */
export const GDELT_LENGTH_REFUSAL = /too short or too long/i;

/** How much of a refusal's text the run log quotes, verbatim. GDELT's
 *  sentences are one line; its 429 text, the longest plain text it has sent
 *  this project, is 444 bytes. */
export const REFUSAL_QUOTE_CHARS = 500;

/**
 * Parse one ArtList body, and say exactly what kind of answer it was:
 *   ok        — EXACTLY two shapes: `{}` with no keys at all (GDELT's "no
 *               matches", seen live 2026-09-26), or a JSON object whose
 *               `articles` is an array (other keys beside it are ignored).
 *               Nothing else is ever read as an answer, so nothing else can
 *               ever be recorded as "no coverage".
 *   refused   — PLAIN TEXT: a body that is not JSON and does not begin the
 *               way JSON ("{" or "[") or markup ("<") begins. GDELT answers a
 *               search it will not run with a sentence instead of results —
 *               "Your query was too short or too long." (measured), "The
 *               specified phrase is too short." (from its documentation's
 *               examples; UNVERIFIED here), or any other — and every such
 *               sentence is a refusal of THAT search, never a failure of the
 *               question (lib/question-press.mjs header, ALL OR NOTHING).
 *               `answer` is the text verbatim (JSON-quoted so it stays one
 *               log line, capped at REFUSAL_QUOTE_CHARS); `forLength` says
 *               whether it was the length sentence.
 *   empty     — a body with nothing in it. Never read as "no matches":
 *               absence is only ever recorded from an answer that says so.
 *   malformed — JSON that does not parse (a truncated body); JSON of any
 *               other shape — an array, a scalar, `{"articles": "nope"}`, and
 *               ANY other object, such as `{"error": "..."}` or
 *               `{"status": "..."}` (an object with keys but no `articles`
 *               list is not the no-match answer, whatever it says); or markup
 *               (an HTML error page from GDELT's front end or a proxy). A
 *               passing failure, not a verdict on the search and never a zero,
 *               so the question is retried as a whole on a later run.
 *
 * Keeps url, domain, seendate and title ONLY. Tone, language, source country,
 * images: never read.
 *
 * @param {string} body
 * @returns {{ ok: true, articles: Array<{ url: string, domain: string, seendate: string, title: string }> } |
 *   { ok: false, kind: 'refused', forLength: boolean, answer: string, error: string } |
 *   { ok: false, kind: 'empty' | 'malformed', error: string }}
 */
export function parseArtList(body) {
  ...
  try {
    ...
    try {
      ...
      if (/^[[{<]/.test(trimmed)) return { ok: false, kind: 'malformed', error: `not JSON: ${head}` };
      const quoted = trimmed.length > REFUSAL_QUOTE_CHARS ? `${trimmed.slice(0, REFUSAL_QUOTE_CHARS)}…` : trimmed;
      const answer = JSON.stringify(quoted);
      return { ok: false, kind: 'refused', forLength: GDELT_LENGTH_REFUSAL.test(trimmed), answer, error: `GDELT answered in plain text instead of results: ${answer}` };
    }
  }
  if (!json || typeof json !== 'object' || Array.isArray(json)) return { ok: false, kind: 'malformed', error: 'not a JSON object' };
  // `{}` and nothing else is "no matches". An object with other keys and no
  // `articles` list ({"error": …}, {"status": …}) is not an answer to the
  // search, so it can never be recorded as zero coverage.
  if (Object.keys(json).length === 0) return { ok: true, articles: [] };
  if (json.articles === undefined) {
    return { ok: false, kind: 'malformed', error: `a JSON object that is neither {} nor an article list: ${head}` };
  }
...
/**
 * Write ONLY WHAT A RUN RECORDED (and repair what another file's change broke).
 *
 *   - A run that recorded no check writes NOTHING (unless the committed file
 *     needs a repair write, the last bullet): not a link that aged out,
 *     not a question that left the live set, not the `as_of` stamp. That is
 *     every run on which each due question failed, was refused on every
 *     term, waited on the circuit or the budget, or had already been checked
 *     today. The file stays exactly as the last run that recorded something
 *     left it, honest as of its own `as_of`: the gate judges it against that
 *     day, its lateness shows up as a gate warning, and the daily digest
 *     raises a ⛔ once no check has been recorded for
 *     QUESTION_PRESS_SILENT_DAYS days (`questionPressActivity`).
 *   - A run that recorded a check writes when the document differs from the
 *     committed one in anything but `as_of`. A recorded check always does on
 *     a new day (its `checkedOn` moves); on the same day it does only when a
 *     question an earlier run could not finish is recorded now, or when the
 *     search itself changed (its `terms`, its links). A question is searched
 *     at most once per UTC day, so that is at most one write per question per
 *     day unless something changed — and a later successful check is never
 *     dropped for having found the same counts, because its `checkedOn` and
 *     `terms` are what the gate's lateness warning reads.
 *   - No file yet: write only when something was recorded (an empty first
 *     file would be a commit and a deploy that records no evidence).
 *   - THE ONE EXCEPTION, A REPAIR WRITE: when the COMMITTED file disagrees
 *     with data/moments.json or data/media-bias.json (`repair` — the gate's
 *     `drift`: a question id moments.json no longer has, an outlet
 *     media-bias.json no longer rates or now rates with another lean), the
 *     run writes even if it recorded no check, and whether or not GDELT
 *     answered. The document it writes is the ordinary one, so the orphaned
 *     question and the un-rated outlet are pruned and a re-rated outlet
 *     moves lean. It moves no `checkedOn`, so the daily digest still reads
 *     the last recorded check correctly. After that write the file agrees
 *     with both, so there is nothing left to repair and the next run writes
 *     only what it records. Without this, a GDELT outage would leave the
 *     disagreement in place for as long as GDELT stayed down.
 *
 * @param {{ previous: any, next: any, recorded: number, repair?: boolean }} input
 *   `recorded` = how many questions this run checked successfully; `repair`
 *   = the committed file carries cross-file drift (`verifyQuestionPress`).
 */
export function shouldWrite({ previous, next, recorded, repair = false }) {
  if (!(recorded > 0) && !(repair && previous)) return false;
  if (!previous) return true;
  const unstamped = (d) => JSON.stringify({ ...d, _meta: { ...(d?._meta ?? {}), as_of: null } });
  return unstamped(previous) !== unstamped(next);
}

// ---- is the collector recording anything? (the daily digest) ------------------

/** A check older than this many whole days is late: the gate warns
 *  (`verifyQuestionPress`). Lateness, never damage (N8-A2). */
export const QUESTION_PRESS_LATE_DAYS = 2;

/**
 * How many days the collector may record no check at all before the daily
 * digest (scripts/pipeline-health.mjs) raises a ⛔. The collector exits 0 on
 * every GDELT outcome — a 429, silence, a refusal, an open circuit — by design
 * (a GDELT outage must never fail anything), so a run's green says nothing
 * about whether it recorded anything; this is the signal that does. Three
 * days is the day the gate's own lateness warning starts (a check more than
 * QUESTION_PRESS_LATE_DAYS old), so the gate and the digest speak up on the
 * same day; with two runs a day it is at least five runs in a row that
 * recorded nothing, not one rate-limited morning.
 */
export const QUESTION_PRESS_SILENT_DAYS = QUESTION_PRESS_LATE_DAYS + 1;

/**
 * Is the collector recording anything? Read from the COMMITTED file — the day
 * of the newest check it holds — because only a recorded check ever moves a
 * `checkedOn`: a run that records nothing writes nothing (`shouldWrite`), and
 * the one exception, a repair write after a moments/bias change, carries
 * every `checkedOn` over unchanged. So that day is the last day any run
 * recorded a check on a question the file still holds. Reads each entry's
 * `checkedOn` and nothing else: no count, no outlet, no link reaches the
 * digest from here.
 *
 * @param {{
 *   data: any,
 *   searchableIds?: string[] | null,
 *   now?: number,
 *   firstRunAt?: string | null,
 * }} input
 *   data          the committed data/question-press.json, or null when it does not exist
 *   searchableIds the live questions that have search terms today (null = not known)
 *   firstRunAt    the oldest collector run the caller can see — only used when no
 *                 check was ever recorded, to tell "never recorded" from "just
 *                 started"; a caller that sees fewer runs than exist can only
 *                 under-alarm, never over-alarm
 */
export function questionPressActivity({ data, searchableIds = null, now = Date.now(), firstRunAt = null }) {
  const today = dayKey(now);
  const alarmDays = QUESTION_PRESS_SILENT_DAYS;
  const entries = data?.questions && typeof data.questions === 'object' && !Array.isArray(data.questions) ? data.questions : {};
  const checked = Object.values(entries)
    .map((e) => e?.checkedOn)
    .filter(seenDayShape)
    .sort();
  const lastChecked = checked.length ? checked[checked.length - 1] : null;
  const silentDays = lastChecked ? daysBetween(lastChecked, today) : null;
  const ids = Array.isArray(searchableIds) ? searchableIds : Object.keys(entries);
  const onRecord = ids.filter((id) => entries[id]).length;
  const lagging = ids.filter((id) => {
    const c = entries[id]?.checkedOn;
    return !seenDayShape(c) || daysBetween(c, today) >= alarmDays;
  });
  const first = Date.parse(firstRunAt ?? '');
  const runningDays = Number.isFinite(first) ? Math.max(0, (now - first) / 86_400_000) : null;
  const nothingToSearch = Array.isArray(searchableIds) && searchableIds.length === 0;
  const never = !nothingToSearch && lastChecked === null && runningDays !== null && runningDays >= alarmDays;
  const silent = !nothingToSearch && ((silentDays !== null && silentDays >= alarmDays) || never);
  return {
    file: Boolean(data),
    lastChecked,
    silentDays,
    searchable: Array.isArray(searchableIds) ? searchableIds.length : null,
    onRecord,
    lagging,
    runningDays,
    nothingToSearch,
...
/**
 * Rated outlets found per lean, against how many domains the AllSides table
 * rates in that lean (`searched` in the return value: every rated domain of
 * the lean could have been admitted) — the denominator matters: the table
 * rates 32 left, 20 center and 25 right domains, so raw counts alone would
 * read a structural difference in the table as a difference in coverage.
 *
...
/**
 * One judgement, three readers, and each reader decides what a finding COSTS:
 *
 *   reader                                  failures   drift          warnings
 *   scripts/check-question-press.mjs (CI)   error      warning        warning
 *   scripts/verify-sync.mjs (the nightly)   warning    warning        warning
 *   scripts/gdelt-intake.mjs, the COMMITTED
 *     file it starts from                   (n/a)      repair write   (n/a)
 *   scripts/gdelt-intake.mjs, the document
 *     it is about to write                  no write   no write       printed
 *
 * The nightly column is all warnings on purpose. This file is OPTIONAL
 * evidence that nothing on the site reads, the nightly never writes it, and
 * failing verify-sync throws away the night's already-paid decodes, coverage
 * and Moment updates (the owner's N8-A2 reasoning, CLAUDE.md). So nothing in
 * this file can ever fail the nightly. Damage still fails CI, where it is
 * caught before it reaches main.
 *
 * `failures` — DAMAGE IN THE FILE ITSELF, which no change to another data
 * file can cause: an unknown schema or window; a missing GDELT citation or
 * link; no recorded query shape or statement of what a count means; any key
 * the format does not define (this is where "never tone, never text" is
 * enforced); a term that breaks the alias rules; a lean that is not a rated
 * lean; a link that is not http(s) on its outlet's own domain, or appears
 * twice in one question; a day outside the window the FILE claims; more links
 * than the per-outlet cap; first/last-seen days that are not the stored
 * links' own; counts that are not the stored evidence's own; a file past its
 * size ceiling.
 *
 * `drift` — THE FILE DISAGREES WITH ANOTHER DATA FILE. Two checks read other
 * files: every question id must exist in data/moments.json, and every outlet
 * must carry the same AllSides lean in data/media-bias.json. A change to
 * either file that deletes a question id, or un-rates or re-rates an outlet
 * this file holds, makes the committed file drift without anything being
 * wrong with it. That is a warning in CI and in the nightly, never a failure,
 * so a moments or bias change can never be blocked by this file and can never
 * fail the nightly. The collector repairs it: a committed file with drift
 * gets a REPAIR WRITE on the collector's next run, even a run on which GDELT
 * answers nothing (`shouldWrite`, `repair`). That write keeps live questions
 * only and re-judges every carried outlet against the current table. The
 * cost, stated plainly: from the check alone, CI cannot tell an outlet
 * media-bias.json STOPPED rating from one that was never rated (a hand edit).
 * Both warn, and the collector's next run removes either. The collector
 * never writes drift itself: its own document must carry none. Retiring
 * a question (a status other than live) is a warning too, and adding a
 * question or a rated outlet changes nothing here.
 *
 ...
 * cannot damage it. Judging it against the clock would fail CI every day from
 * 00:00 UTC until the first collector run of the day rewrote it. That would
 * be exactly the "we are behind" check the owner's N8-A2 ruling keeps out of
 * the gates (CLAUDE.md), so lateness here is a WARNING only. The only
 * wall-clock FAILURE is an `as_of` more than a day in the future, which no
 * honest writer produces.
 *
 * `warnings` — a question that is no longer live (the next write drops it); a
 ...
 * @returns {{ failures: string[], drift: string[], warnings: string[], notes: string[] }}
 */
export function verifyQuestionPress({ data, fileBytes = 0, bias = null, moments = null, now = Date.now() }) {
  ...
  const REPAIR = "the collector's next run removes it (a repair write)";
  const LATE_DAYS = QUESTION_PRESS_LATE_DAYS;

  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    fail('question-press: not a JSON object');
    return { failures, drift, warnings, notes };
  }
  ...
  for (const [id, entry] of Object.entries(questions)) {
    ...
    if (moments) {
      if (!moments[id]) drift.push(`${at}: no such question in data/moments.json — ${REPAIR}`);
    ...
    for (const o of entry.outlets) {
      ...
      if (bias) {
        ...
        if (!rated) drift.push(`${oat}: data/media-bias.json carries no AllSides rating for it (${OUTLET_POLICY}) — ${REPAIR}`);
        else if (rated !== o.lean) drift.push(`${oat}: lean "${o.lean}" disagrees with data/media-bias.json ("${rated}") — the collector's next run moves it (a repair write)`);
      }
