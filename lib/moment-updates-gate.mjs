 ...
 * Congress.gov's "(No short title on file)", and the "unchanged" / "sin
 * cambios" that follows a passage verb.
 * @param {string} text
 * @param {'en'|'es'} lang
 * @returns {string[]}
 */
 ...
 * @param {{ now?: number, fileBytes?: number, pressOutletAdmits?: (domain: string) => boolean, pressOutletRated?: (domain: string) => boolean }} [opts]
 ...
 *   allowlist. scripts/check-moment-updates.mjs always supplies it; it is an
 *   option only so this module keeps its one import and the fixture suites
 *   that predate the floor keep their call shape.
    ...
    updates.forEach((u, i) => {
      ...
      if (u.class === 'press_cluster') {
        ...
        if (typeof opts.pressOutletAdmits === 'function') {
          const refused = outlets.filter((o) => !opts.pressOutletAdmits(o));
          if (refused.length) {
            violations.push(
              `${up}.source.outlets: names ${refused.join(', ')} — a press update names only AllSides-rated outlets (or the owner's allowlist), never an unrated one`,
            );
          }
        }
