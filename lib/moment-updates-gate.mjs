 ...
 *
 * ---------------------------------------------------------------------------
 * THE EDITORIAL LAW (owner-settled 2026-07-25, docs/ideation/
 * 2026-07-25-moments-v2.md §2) — this module is its enforcement surface:
 *
...
export function identityKey(update) {
  ...
  if (update?.record) {
    const code = update.record.action_code || update.record.action_type || '';
    const text = normalizeText(update.record.action_text).slice(0, ACTION_TEXT_KEY_CHARS);
    return `act:${String(code).toLowerCase()}:${text}`;
  }
...
export function pruneEntry(entry, opts = {}) {
  ...
  const updates = capped.sort(compareUpdates).slice(0, MAX_UPDATES_PER_MOMENT);

  const revisions = [...(entry.summary_revisions ?? [])].slice(-MAX_REVISIONS);
 ...
 */
const SPECULATION = {
  en: /\b(expected to|likely to|could|might|set to|poised to|on track to)\b/i,
  es: /\b(se espera|probablemente|podría|podrían|estaría|estarían|previsto que|a punto de)\b/i,
};

const SPECULATION_LABEL = {
  en: 'expected to / likely to / could / might / set to / poised to / on track to',
  es: 'se espera / probablemente / podría / podrían / estaría / estarían / previsto que / a punto de',
};
