 ...
 */
export function mapNominationStatus(actionText) {
  ...
  //     FUTURE calendar date, so it must not be flattened into `floor`.
  if (/\bunanimous consent agreement\b/i.test(text)) return 'scheduled';
  ...
  // (6) "Motion by Senator Thune to reconsider tabled in Senate by Yea-Nay
  //     Vote. 51 - 44. Record Vote Number: 220." (1) — a floor proceeding
  //     that carries a yea-nay vote but is NOT a confirmation. Deliberately
  //     narrow: a generic `yea-nay vote` rule here would re-create exactly
  //     the bill mapper's failure for any future sentence shape, so anything
  //     with a recorded vote that is neither a confirmation nor a
  //     reconsideration falls through to `unclassified` rather than being
  //     asserted as live floor business.
