...
/**
 * The published qualifying window, in days.
 *
 * /questions and /questions/[id] tell users in both languages that a signal
 * counts when it landed "within the last 14 days" (`moments.whyCriteria`,
 * `moments.howMadeRule2`). This constant is that promise, in one place, so
 * the claim on the page and the behaviour in the code cannot drift apart —
 * which is exactly what had happened: amber rendered on floor-calendar dates
 * 31 and 39 days old, on a page publishing a 14-day rule directly beside it.
 *
