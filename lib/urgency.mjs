/*
 * The urgency curve — the ONE copy. Imported by lib/data.ts (the live site's
 * feed ordering), scripts/sync-coverage.mjs (which must agree with the site
 * on which bills are "top band"), and scripts/sync-bills.mjs (which shares
 * STATUS_BASE for its stored sync-time score). Plain .mjs with JSDoc types so
 * the TS lib, the node scripts, and the unit test all import it unchanged.
 *
 ...
 * @returns {number}
 */
export function effectiveUrgency(status, lastActionDate) {
