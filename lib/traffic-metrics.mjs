...
// candidate.** scripts/moment-watch.mjs reads `--state all` when it decides
// whether a bill has already been filed, and scripts/moment-approve.mjs
// reads the close as the decline signal. An auto-close on that label would
// not be tidying — it would silently cast the owner's vote against
// publishing a Moment, and it would look exactly like he had decided. The
// same shape of harm applies to every other bot label in the repo, so the
// denylist below is a hard gate rather than a heuristic.
