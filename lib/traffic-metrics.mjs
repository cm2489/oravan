...
// median (~6/day). Stated precisely so nobody has to re-derive it: 150
// retires three of the four alerts (#127–#129, at 76/79/78) — the fourth,
// #130's 165-call peak, would STILL fire under the new floor. On
// 2026-07-30 the trailing-7-day median was 28 (the plateau was only four
// days old and the week before it near-zero), so 165 cleared both 150 and
// 3× median. That is the intended behavior, not residual noise: a genuine
// order-of-magnitude burst should alarm once. Pinned by the "165 peak
// still fires" test in tests/traffic-metrics.unit.spec.ts.
...
// --- the site page-view section (site-counter, 2026-09) ---------------------

/**
 * The one-line caveat that rides under the site block, every day. It says
 * the three things a number like "412" would otherwise be read as meaning
 * and does not: these are REQUESTS (a reload is two), bots are in them, and
 * no unique/visitor count exists — the counters carry no identity to build
 * one from, and whether one should exist is a pending owner ruling, not an
 * unfinished feature.
 */
export const SITE_PAGEVIEW_CAVEAT =
  '_Site page views are requests, bots included — not people, and not unique visitors ' +
  '(no unique/visitor count exists: the counters hold one integer per route template per ' +
  'UTC day and no visitor identity at all). A reload counts twice; client-side navigations ' +
  'and prefetches count zero. Unmatched paths, including 404s, land in `other`._';

/**
 * The site block's lines: one per route template, then a total. Descriptive
 * only — NO spike check and no decline check on this series, deliberately.
 * Both detectors were calibrated against MCP/script volumes with recorded
 * bases (MCP_SPIKE_FLOOR's own comment shows what an uncalibrated floor
 * costs in false alarms), and this family has no history yet to calibrate
 * against. It reports; it does not alarm. Revisit once there are weeks of
 * real numbers to set a floor from.
 *
 * @param {Array<{ surface: string, stats: SeriesStats }>} surfaces - PAGEVIEW_SURFACES order
 * @param {SeriesStats} [total] - the summed series; omitted renders no total line
 * @returns {string[]} the block's lines, or a single "not computed" line when
 *   no window was supplied — never silently absent, same rule as the trend line
 */
export function formatSiteLines(surfaces, total) {
  if (!surfaces || surfaces.length === 0) {
    return ['Site page views: not computed (no page-view window supplied)'];
  }
  const nameWidth = Math.max(...surfaces.map((s) => s.surface.length), '── total'.length) + 2;
  const lines = surfaces.map(
    ({ surface, stats }) =>
      `  ${pad(surface, nameWidth)}${pad(stats.latest, 8)}(7d median ${stats.med}, WoW ${formatPercent(stats.wow)})`
  );
  if (total) {
    lines.push(
      `  ${pad('── total', nameWidth)}${pad(total.latest, 8)}(7d median ${total.med}, WoW ${formatPercent(total.wow)})`
    );
  }
  return lines;
}
 ...
 * @param {Array<{ surface: string, stats: SeriesStats }>} [input.sitePageviews] - day-1 site page views per route template (lib/usage.ts readPageviewWindow); descriptive only, never spike-checked
 * @param {SeriesStats} [input.siteTotal] - the summed page-view series
...
export function formatDigestBody({
  ...
  return [
    ...
    scriptLine,
    '',
    'Site page views (production, by route template)',
    ...formatSiteLines(sitePageviews, siteTotal),
    '```',
    '',
    // Corrected 2026-09 (site-counter). This paragraph used to open "Site
    // page-view traffic: not measured" and explain which vendor route was
    // closed to us; the block above is now that measurement, so it states
    // where the numbers come from instead. Merged with the 2026-09-18
    // correction from main: Vercel's server-side Observability is NOT
    // "no REST API" — `vercel metrics --format json` (CLI, public beta 2026,
    // needs Observability Plus) and runtime-log aggregation both expose those
    // numbers without a client script. The @vercel/analytics ban is unchanged.
    'Site page views above are FIRST-PARTY and server-side (`lib/usage.ts`): one ' +
      'counter per route template per UTC day, incremented in `proxy.ts`, holding no ' +
      'path, query, locale, referer, IP, User-Agent, or cookie. Not Vercel Web ' +
      'Analytics — that API needs the @vercel/analytics client script, which CLAUDE.md ' +
      'permanently bans. A vendor server-side source also exists — `vercel metrics ' +
      '--format json` (Observability Plus), no client script — and this digest ' +
      'does not read it yet; the first-party counter needs no plan tier and no new token.',
    '',
    SITE_PAGEVIEW_CAVEAT,
    '',
...
// --- issue hygiene (2026-08) -------------------------------------------------
//
// The daily digest is the one job that already looks at this repo's issue
// list every morning, so it is where issue hygiene belongs — no new
// workflow, no new schedule, no new permission.
//
// TWO THINGS, and the asymmetry between them is the whole design:
//
//   1. It CLOSES stale traffic-spike issues. A spike is an event that is
//      over by the next morning (see the standing-decline comment above for
//      the event-vs-state split); a week later the issue is a stale alert
//      nobody will action, and the running series it describes is already
//      in the digest.
//
//   2. It LISTS everything else. It never closes anything else, ever.
//
// The reason rule 2 is absolute, stated here so nobody relaxes it later:
// **closing a `moment-candidate` issue is how the owner DECLINES a
// candidate.** Two places in the repo say so directly:
//
//   - the candidate issue's own body instructs him to do it —
//     "### To decline / Close this issue and append the reason to
//     `docs/moment-rejections.json`" (scripts/moment-watch.mjs);
//   - moment-watch.yml's filed-check lists `--label moment-candidate
//     --state all` precisely so a CLOSED issue still counts as filed, with
//     the comment "closing an issue is how the owner declines a candidate."
//
// So an auto-close on that label would not be tidying. It would perform the
// documented decline gesture on the owner's behalf, look exactly like he had
// decided, AND permanently suppress the candidate from ever being re-filed.
// The same shape of harm applies to every other bot label in the repo, so
// the denylist below is a hard gate rather than a heuristic.

/**
 * Labels this job will NEVER close, whatever else an issue looks like.
 *
 * Enumerated FROM THE REPO (grep `gh label create` / `--label` across
 * .github/workflows/ and app/api/feedback/route.ts), not from memory, and
 * deliberately over-broad: a label on this list can only ever PREVENT a
 * close, never cause one, so the cost of an extra entry is zero and the
 * cost of a missing one is an issue closed out from under its owner.
 *
 *   moment-candidate    CLOSING IS THE OWNER'S "DECLINE" VOTE (see above).
 *   approve-moment      the owner's publish trigger, applied by hand to a
 *                       candidate issue — moment-approve.yml acts on it.
 *   moment-review       the standing weekly Moment shortlist digest.
 *   metrics             the pinned digest issue this job posts INTO.
 *   traffic-decline     the standing decline issue, which has its own
 *                       lifecycle above and closes itself when the state
 *                       clears — never on a timer.
 *   redistricting-watch a tracked-state map change awaiting manual review.
 *   data-vacancy        a vacant House seat, never silently backfilled —
 *                       the one label in this repo created out-of-band.
 *   journey-corpus      a floor sentence no matcher reads yet; the standing
 *                       issue is the backlog itself.
 *   beta-feedback       a real person wrote it. Nothing automated closes a
 *                       human's message.
 *   bug/feature/        the beta-feedback category labels
 *   partnership/other   (app/api/feedback/route.ts CATEGORIES).
 *   enhancement         GitHub's default label, used by hand.
 */
export const NEVER_CLOSE_LABELS = [
  'moment-candidate',
  'approve-moment',
  'moment-review',
  'metrics',
...
/**
 * Labels whose open issue is a STANDING fixture rather than a task: it is
 * meant to stay open forever, and its presence in an "awaiting your word"
 * list would be permanent noise that teaches the reader to skip the list.
 *
 * Only these two. The other standing-SHAPED issues in this repo —
 * redistricting-watch, journey-corpus, traffic-decline — each carry real
 * work or a live alarm (review a map change, write a matcher, traffic is
 * down), so they stay in the actionable list on purpose. "Standing" here
 * means "recurring digest", not "long-lived".
 */
export const STANDING_LABELS = ['metrics', 'moment-review'];

/**
 * How long a traffic-spike issue stays open. BASIS: a spike alert is
 * point-in-time — it reports ONE day's count against that day's trailing
 * median, and by the next morning the number it describes is history. The
 * running series (every day's totals, the median, WoW, the 28-day trend)
 * is in the daily digest, which is a strictly better place to notice a
 * level that stayed high. A week is long enough for the owner to see the
 * alert on a normal cadence and short enough that the issue list never
 * accumulates dated alerts nobody will action — the exact failure the
 * MCP_SPIKE_FLOOR recalibration above already retired four of (#127–#130).
 */
export const SPIKE_ISSUE_TTL_DAYS = 7;

/**
 * The EXACT shape spikeIssueContent() writes — `Traffic spike: <series> —
 * <YYYY-MM-DD>`, em dash and all. Anchored at both ends so a title with a
 * hand-added suffix ("… — 2026-08-01 (still happening?)") does not match:
 * once a human has edited the title, the issue is not this job's to close.
 * Pinned against spikeIssueContent in tests/traffic-metrics.unit.spec.ts so
 * the two can never drift.
 */
export const SPIKE_ISSUE_TITLE_RE =
  /^Traffic spike: (total MCP calls|script generations) — (\d{4}-\d{2}-\d{2})$/;

/** `gh issue list --json labels` gives objects; be tolerant of bare strings. */
...
/** UTC-midnight epoch ms for a YYYY-MM-DD string, or null when the string is
 *  shaped like a date but is not one (2026-13-45 passes \d{4}-\d{2}-\d{2}). */
...
/**
 * Whole days elapsed since an ISO instant (a full `createdAt` timestamp or
 * a bare YYYY-MM-DD, which reads as that day's UTC midnight). `null` when
 * the string will not parse — the callers treat that as "unknown", which is
 * never "old enough to close".
 *
 * A future timestamp (clock skew between GitHub and the runner) reports 0
...
export function ageInDays(iso, now) {
  const t = /^\d{4}-\d{2}-\d{2}$/.test(iso ?? '') ? parseIsoDateUtc(iso) : Date.parse(iso ?? '');
...
/**
 * @typedef {Object} ClosableSpikeIssue
 * @property {number} number
 * @property {string} title
 * @property {string} series - 'total MCP calls' | 'script generations'
 * @property {string} reportedDate - the date IN THE TITLE, not createdAt
 * @property {number} ageDays - days since reportedDate
 */

/**
 * The open issues this job may close: stale traffic-spike alerts, and
 * NOTHING else. Three gates, ALL of which must pass. They are listed here
 * in reading order; the CODE evaluates the denylist second, immediately
 * after the label check and before the title match, so no later edit can
 * slip a match ahead of the veto:
 *
 *   (a) LABEL — carries `traffic-spike`. This job only ever cleans up its
 *       own alerts.
 *   (b) TITLE — matches SPIKE_ISSUE_TITLE_RE exactly, anchored at both
 *       ends. An edited title means a human took the issue over.
 *   (c) DENYLIST — carries no NEVER_CLOSE_LABELS label. The denylist WINS
 *       over (a) and (b): an issue labelled both `traffic-spike` and
 *       `moment-candidate` is never closed, because guessing wrong there
 *       silently casts the owner's decline vote (see this section's header).
 *
 * Staleness is measured from the date REPORTED IN THE TITLE, not from
 * `createdAt`. They are normally the same morning, but the title's date is
 * the one the alert is actually about — the day whose count spiked — and a
 * re-filed or hand-moved issue must not get a fresh week of life from its
 * creation timestamp.
 *
 * @param {Array<{number: number, title: string, labels?: Array<{name: string}|string>, createdAt?: string}>} issues
 * @param {{ now: Date }} options
 * @returns {ClosableSpikeIssue[]} in the input's order
 */
export function closableIssues(issues, { now }) {
  const out = [];
  for (const issue of Array.isArray(issues) ? issues : []) {
    const names = labelNames(issue);
    // (a) our own alert label
    if (!names.includes('traffic-spike')) continue;
    // (c) denylist first in spirit — checked here so no later edit can
    // reorder a match ahead of it; a protected label vetoes everything.
    if (names.some((n) => NEVER_CLOSE_LABELS.includes(n))) continue;
    // (b) exactly the title this job writes
    const m = SPIKE_ISSUE_TITLE_RE.exec(typeof issue.title === 'string' ? issue.title : '');
    if (!m) continue;
    const reportedDate = m[2];
    const ageDays = ageInDays(reportedDate, now);
    if (ageDays === null || ageDays <= SPIKE_ISSUE_TTL_DAYS) continue;
    out.push({ ...issue, series: m[1], reportedDate, ageDays });
  }
  return out;
}

/**
 * The closing comment on a stale spike issue. Says WHY it closed, that the
 * close was automatic, and where the ongoing number lives — so the close
 * reads as bookkeeping rather than as a verdict that the traffic was fine.
 *
 * @param {Object} input
 * @param {string} input.date - the date reported in the issue's title
 * @param {number} input.ageDays - days between that date and today
 * @param {string|number} [input.digestIssue] - the pinned digest issue's
 *   number, passed in by the caller. Omitted rather than guessed when the
 *   caller does not know it.
 * @returns {string}
 */
export function spikeClosedComment({ date, ageDays, digestIssue }) {
  const where = digestIssue
    ? `the daily digest on issue #${digestIssue}`
    : 'the pinned daily-metrics digest issue';
  return [
    `<!-- traffic-spike-closed:${date} -->`,
    `**Closed automatically — ${ageDays} days after the reported date (${date}).**`,
    '',
    'A traffic spike is a point-in-time alert: it reports one day\'s count against ' +
      'that day\'s trailing median, and the day it describes is over. Left open past ' +
      `${SPIKE_ISSUE_TTL_DAYS} days it is a dated notice nobody will action — the exact ` +
      'noise the spike floor was recalibrated to stop producing.',
    '',
    `The running series — every day's totals, the 7-day median, the week-over-week ` +
      `comparator and the 28-day trend — lives in ${where}. A level that STAYED high ` +
      'shows up there, and a level that fell has its own standing issue. This close ' +
      'is bookkeeping, not a finding. Reopen it if you want the thread back.',
    '',
    `Tunable: \`SPIKE_ISSUE_TTL_DAYS = ${SPIKE_ISSUE_TTL_DAYS}\` — see \`lib/traffic-metrics.mjs\`.`,
  ].join('\n');
}

/**
 * The digest's issue-hygiene section: what is open and waiting on the
 * owner, and — on its own line — what is open forever by design and is NOT
 * waiting on him.
 *
 * The standing set is derived from STANDING_LABELS, never from issue
 * numbers: #81 and #177 are today's numbers, and the day either is recreated
 * a hardcoded list would quietly start reporting the wrong thing. The
 * numbers still get PRINTED (they are what you click), they are just never
 * what the code matches on.
 *
 * Prints every day, including "(0) — nothing is waiting on you", for the
 * same reason the 28-day trend line prints every day: a line that only
 * appears when there is something wrong is indistinguishable from a line
 * whose check quietly broke. The one case that renders nothing at all is
 * the caller having no data — and the caller handles that by not calling
 * this, so an absent section always means the read failed and said so.
 *
 * @param {Array<{number: number, title: string, labels?: Array<{name: string}|string>, createdAt?: string}>} issues
 * @param {{ now: Date }} options
 * @returns {string}
 */
export function awaitingYourWordSection(issues, { now }) {
  const standing = [];
  const actionable = [];
  for (const issue of Array.isArray(issues) ? issues : []) {
    const names = labelNames(issue);
    const standingNames = names.filter((n) => STANDING_LABELS.includes(n));
    if (standingNames.length > 0) standing.push({ issue, standingNames });
    else actionable.push({ issue, names, ageDays: ageInDays(issue.createdAt, now) });
  }

  // Newest first, so the list reads in the order the owner met these; ties
  // broken by descending number. Explicitly sorted rather than trusting
  // whatever order `gh` returned, so the same issue set always renders the
  // same bytes.
  actionable.sort((a, b) => (a.ageDays ?? 0) - (b.ageDays ?? 0) || b.issue.number - a.issue.number);
  standing.sort((a, b) => a.issue.number - b.issue.number);

  const standingLine =
    standing.length === 0
      ? ''
      : `Standing by design (${standing.length}): ` +
        standing.map((s) => `#${s.issue.number} ${s.standingNames.join('/')}`).join(' · ') +
        ' — not awaiting anything.';

  if (actionable.length === 0) {
    return ['---', '', '**Awaiting your word (0) — nothing is waiting on you.**', ...(standingLine ? ['', standingLine] : [])].join(
      '\n'
    );
  }

  const lines = actionable.map(({ issue, names, ageDays }) => {
    const label = names.length > 0 ? names.join(', ') : 'no label';
    const age = ageDays === null ? 'age unknown' : `${ageDays}d open`;
    return `- #${issue.number} · ${issue.title} · ${label} · ${age}`;
  });

  return [
    '---',
    '',
    `**Awaiting your word (${actionable.length})** — open issues that are not standing by design:`,
    '',
    ...lines,
