...
export function formatDigestBody({
  ...
  return [
    ...
    // where the numbers come from instead. The vendor half is kept because
    // it is still true and still the reason this is first-party.
    'Site page views above are FIRST-PARTY and server-side (`lib/usage.ts`): one ' +
      ...
      'permanently bans, and Vercel\'s server-side Observability (Edge Requests, no ' +
      'client script) has no REST API, dashboard-only.',
    '',
