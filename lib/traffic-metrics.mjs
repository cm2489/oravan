 ...
 * 7-day BLOCK SUMS, never percentages and never medians — a median of a
 * week of 4s and 6s is 4 or 5 either way, which says nothing about whether
 * a hundred-a-day channel died. Sums do.
 *
 ...
 * instrument here by construction: the week that broke this product's MCP
 * traffic (609 calls → 32) has a trailing median in the single digits on
 * BOTH sides of the break, so a median-based test reads a 95% collapse as
 * "roughly flat." Percentages have the same failure in the other direction:
 * 2 calls against 1 is +100%.
 *
