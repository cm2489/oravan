...
export function formatDigestBody({
  ...
  return [
    ...
    '```',
    '',
    // Corrected 2026-09-18. This line used to end "Vercel's server-side
    // Observability (Edge Requests, no client script) has no REST API —
    // dashboard-only", which had stopped being true: `vercel metrics
    // --format json` (CLI, public beta 2026, needs Observability Plus) and
    // runtime-log aggregation over the API both expose those numbers without
    // a client script. The ban on @vercel/analytics is unchanged and still
    // the reason page views are not here today; what changed is that the
    // second half was a false "there is no way", and a digest that files an
    // untrue excuse every morning is worse than one that names the gap.
    'Site page-view traffic: not measured HERE. Vercel\'s Web Analytics API needs the ' +
      '@vercel/analytics client script, which CLAUDE.md permanently bans. A compliant ' +
      'server-side source does exist — `vercel metrics --format json` (Observability ' +
      'Plus) and runtime-log aggregation, neither needing a client script — and this ' +
      'digest does not read it yet. See the pinned issue\'s first comment for the full note.',
    '',
