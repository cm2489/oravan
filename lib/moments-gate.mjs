...
export const SIGNAL_TYPES = [
  'tier0_floor',
  // Floor ACTION — a motion or cloture filing on the floor, which is not the
  // same fact as a calendar placement and must not wear its label. See
  // lib/moments.ts's QUALIFYING_SIGNAL_TYPES for the full argument and for
  // why it is chamber-neutral.
...
/**
 * The STORED moment statuses, and only those: `settled` is COMPUTED at read
 * time (lib/moments.ts computeMomentState) and must never appear in
 * data/moments.json.
 *
 * Extracted from two inline literals 2026-08-12 for the same reason
 * COVERAGE_TIERS was extracted in lib/types.ts: scripts/moment-draft.mjs's
 * enumLeaks() keeps a hand-written copy of every internal vocabulary, a
 * hand-written copy of a list that exists nowhere as a list cannot be pinned,
 * and an unpinned copy silently went one token short for three days
 * (tier0_floor_action). tests/moment-draft.unit.spec.ts asserts the superset.
 */
export const MOMENT_STATUSES = ['live', 'retired'];

/**
 * THE LIVE CAP — the most stored-live moments the gate lets through (spec
 * §4.3: scarcity keeps curation honest). It is ALSO a reader-facing claim:
 * `moments.scarcityNote` prints "never more than {cap}" on the homepage band,
 * /questions and every question page, and those pages read this constant
 * (re-exported by lib/moments.ts) rather than a number typed into the
 * message. So for the GATE and the PAGES, raising the cap here is the whole
 * change: the printed promise cannot drift from the rule that enforces it.
 * The unit suite pins the gate against this value at exactly LIVE_CAP and
 * LIVE_CAP + 1.
 *
 * NOT YET ONE HOME FOR THE PIPELINE. Three scripts still carry their own
 * copy of the number, and until they import this constant each must be
 * changed with it: scripts/moment-approve.mjs (`export const LIVE_CAP`, the
 * slot math for approvals), scripts/moment-candidates.mjs (`const LIVE_CAP`,
 * whose comment still points at the gate's old `liveCount > N` literal rather
 * than at this constant) and scripts/moment-watch.mjs (the "(N live)" floor
 * reason). This file's header states the number in prose too.
 */
 ...
 * `\b` cannot do this job for accented Spanish. Composes a source string into
 * a `u`-flagged, case-insensitive regex.
 *
 * EXPORTED so lib/moment-updates-gate.mjs's speculation lint (layer 2) can use
 * the same boundary as this vocabulary table (layer 1). The two lints run over
 * the same strings on the same nightly path, and a boundary that means one
 * thing in one layer and another in the next is how "luchó" passed layer 1 for
 * three years. One definition, imported, never re-derived — the §2.3
 * discipline this module already applies to the word table itself.
 */
export const esWord = (source) => new RegExp(`(?<!\\p{L})(?:${source})(?!\\p{L})`, 'iu');

/**
 * Accented Spanish, spelled either way, from ONE canonical term.
 *
 * A diacritic-dropping model — or a hurried author — writes "podria" for
 * "podría", and an accent-exact pattern simply does not see it. Rather than
 * maintain two spellings of every term by hand (which is a drift surface: the
 * next term added gets one spelling and the lint quietly half-works), each
 * accented vowel is expanded to a class matching both. `ü` and `ñ` are here
 * for completeness, not because any current term needs them.
 *
 * Terms are plain words and spaces, never regex metacharacters — this escapes
 * nothing, on purpose, so a term carrying a metacharacter fails loudly in a
 * test rather than matching something surprising in production.
 *
 * DELIBERATELY NOT APPLIED TO FORBIDDEN.es BELOW, and the reason is not
 * oversight. Folding `ó` to `[oó]` across that table turns `salv(…|ó|…)` into
 * a matcher for "salvo", which is the ordinary Spanish word for "except" and
 * is the documented false-positive this table was tuned to avoid; `luch(…)`
 * would likewise start matching the first-person "lucho". The speculation
 * terms have no such collisions — there is no Spanish word "podria" or
 * "estaria" other than the misspelled hedge — which is why the fold is safe
 * there and would be a regression here. Two lints, two judgements, both
 * written down.
 */
const ACCENT_CLASS = { á: 'aá', é: 'eé', í: 'ií', ó: 'oó', ú: 'uú', ü: 'uü', ñ: 'nñ' };
export const eitherSpelling = (term) =>
  String(term).replace(/[áéíóúüñ]/g, (ch) => `[${ACCENT_CLASS[ch]}]`);
...
/**
 * A moment id: lowercase kebab, ASCII only, no leading/trailing/doubled dash.
 *
 * EXPORTED since 2026-08-12 so scripts/moment-approve.mjs can reject a bad id
 * at the point it PARSES one out of a GitHub issue, rather than only here at
 * the end. The two uses are not redundant, and the difference is what the id
 * is between them: this file validates a key that is already in the file,
 * while that one validates a key an untrusted issue body proposed — and it
 * then puts that key in a branch name, a commit message, a `$GITHUB_OUTPUT`
 * line and a PR title. An id shaped like `evil\nreason=$(id)` is not a schema
 * problem there, it is a shell one.
 *
 * Exported rather than copied for the §2.3 reason the whole gate is built on:
 * a second regex is a second definition, and the day they disagree the loose
 * one wins.
 */
...
export function checkMoments(moments, slugsByKind, statusFor, opts = {}) {
  ...
  for (const [id, m] of Object.entries(moments)) {
    ...
    // reviewed: OPTIONAL (2026-09-24) — the day a person last re-read the
    // prose and merged it. When present it must be a real date no earlier
    // than `opened` and not in the future: it is printed on the page as
    // "last reviewed", so a typo would be a false claim about our own upkeep.
    if (m.reviewed !== undefined) {
      if (!isNonEmptyString(m.reviewed) || !DATE_RE.test(m.reviewed) || !Number.isFinite(new Date(m.reviewed).getTime())) {
        violations.push(`${at('reviewed')}: when present, must be a parseable YYYY-MM-DD date`);
      } else if (isNonEmptyString(m.opened) && m.reviewed < m.opened) {
        violations.push(`${at('reviewed')}: ${m.reviewed} is before opened (${m.opened})`);
      } else if (new Date(m.reviewed).getTime() > now) {
        violations.push(`${at('reviewed')}: ${m.reviewed} is in the future`);
      }
    }
    if (
      m.status === 'live' &&
      ...
      warnings.push(`${at('review_by')}: ${m.review_by} has passed — it stays on the site (review dates no longer hide a question) and the nightly moment-watch run flags it until a reviewed PR renews or retires it`);
    }

    // stored status: live | retired only ('settled' is computed, never stored)
    if (!MOMENT_STATUSES.includes(m.status)) {
      violations.push(`${at('status')}: "${m.status}" — stored status must be ${MOMENT_STATUSES.map((s) => `"${s}"`).join(' or ')} (settled is computed at read time, never stored)`);
    }
    if (m.status === 'live') liveCount++;
  }

  if (liveCount > LIVE_CAP) {
    violations.push(`data/moments.json: ${liveCount} live moments — the cap is ${LIVE_CAP} (scarcity keeps curation honest, spec §4.3)`);
  }
