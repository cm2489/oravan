 ...
 *   - cap: at most 8 stored-live moments (was 6 until the owner's 2026-09-24
 *     ruling to keep all six live questions AND publish a seventh; the
 *     scarcity rationale of spec §4.3 is unchanged — only the number moved,
 *     with one slot of headroom so the next ruling is not forced by the cap)
...
/** How many Big Questions may be live at once. The one copy the pages read (the
 * "never more than N" line); raised from 6 by owner ruling 2026-09-24. */
...
export function checkMoments(moments, slugsByKind, statusFor, opts = {}) {
  ...
  for (const [id, m] of Object.entries(moments)) {
    ...
    // reviewed: OPTIONAL (2026-09-24) — the day the entry's text last
    // changed. When present it must be a real date no earlier than `opened`
    // and not in the future: it is printed on the page as "Summary updated
    // {date}", so a typo would be a false claim about our own upkeep.
  ...
  if (liveCount > LIVE_CAP) {
    violations.push(`data/moments.json: ${liveCount} live moments — the cap is ${LIVE_CAP} (scarcity keeps curation honest, spec §4.3; raised from 6 by owner ruling 2026-09-24)`);
  }
