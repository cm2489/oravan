 ...
 *                  RETIRES ON ONE THING ONLY (owner decision D13, 2026-09-18):
 *                  the bill's own record showing that the announcing chamber
 *                  has already voted, dated inside the week the announcement
 *                  covers. See `announcementAnswered`.
...
/**
 * MERGE THE TWO "WHEN DID WE LAST CHECK" STAMPS.
 *
 * data/floor-signals.json's own `_meta.fetched_at` only moves when the file's
 * CONTENT changes or the file itself was aged-and-restamped WHILE at least one
 * bill signal was live (scripts/floor-signals-parse.mjs's `shouldWrite`) — a
 * deliberate choice, pinned by tests/floor-signals.unit.spec.ts, so a genuine
 * recess produces zero commits. That leaves the in_session verdict, the
 * next-meeting claim and the "as of" copy (`floorSourcesPosture`,
 * `chamberSessionFrom` below, `floorSignalsCheckedAt`) reading a stale stamp
 * on any hour where the schedule genuinely didn't move AND no bill signal was
 * live — the common "light in-session week" case, not the recess case.
 *
 * data/floor-signals-checked.json (scripts/floor-signals-parse.mjs's
 * `shouldWriteChecked`) exists only to carry that reconfirmation, decoupled
 * from bill-signal liveness. This function is the one place that reads both
 * stamps and picks whichever is newer, so every caller benefits from a single
 * merge rather than re-deriving it.
 *
 * @param {any} meta  data/floor-signals.json's `_meta`
 * @param {any} [checked]  data/floor-signals-checked.json, or null/undefined
 * @returns {{ fetched_at: string | null }}
 */
export function floorSignalsHealthy(meta, checked) {
  const metaStamp = Date.parse(meta?.fetched_at ?? '');
  const checkedStamp = Date.parse(checked?.checked_at ?? '');
  const metaOk = Number.isFinite(metaStamp);
  const checkedOk = Number.isFinite(checkedStamp);
  if (!metaOk && !checkedOk) return { fetched_at: null };
  if (checkedOk && (!metaOk || checkedStamp > metaStamp)) return { fetched_at: checked.checked_at };
  return { fetched_at: meta?.fetched_at ?? null };
}
 ...
 * FLOOR, which is strictly further along than "a vote is pending".
 *
 * AMENDED 2026-09-24. "Measure laid before Senate" is no longer one of the
 * T1-only sentences: S. 4668 sat on "Considered by Senate." the morning of its
 * cloture vote, and a measure under consideration has its vote still ahead in
 * the chamber considering it, so floorPendingChamber now reads the whole
 * consideration family (SENATE_/HOUSE_/CHAMBER_SILENT_CONSIDERATION in
 * lib/floor-text.mjs) and this function admits the same shapes. The two
 * T1-only sentences are now "cloture … invoked" and "Motion to proceed to
 * measure considered in Senate". The settled
...
export function entersFloorWatch(actionText) {
  ...
  return (
    ...
    // The House ADOPTING that rule (floorPendingChamber's rule 5, issue
    // #268). Mirrored here because T1 must stay a superset of the crown's
    // pending gate — tests/docket.unit.spec.ts pins that over the corpus.
    /\brule\s+h\.\s?res\.\s*\d+\s+passed\s+house\b/i.test(t) ||
    /cloture .*invoked/i.test(t) ||
    /motion to proceed to (?:the )?measure considered in senate/i.test(t) ||
    // THE CONSIDERATION SENTENCES (2026-09-24, S. 4668) — the same three
    // groups floorPendingChamber now reads, taken from the ONE copy in
    // lib/floor-text.mjs so the superset cannot drift. "Measure laid before
    // Senate" was already admitted here (the SEED Act, above); it is now in
    // SENATE_CONSIDERATION and the crown reads it too. The chamber-silent
    // group is admitted WITHOUT the page citation the crown asks for: this is
    // a ranking position, and "Considered as unfinished business." is a
    // measure physically on a floor whichever chamber wrote it.
    SENATE_CONSIDERATION.test(t) ||
    HOUSE_CONSIDERATION.test(t) ||
    CHAMBER_SILENT_CONSIDERATION.test(t)
  );
}
 ...
 *
 * Reads the sentence the status was read from (statusBasisText,
 * lib/floor-text.mjs): a defeat stored behind "Motion to reconsider laid on
 * the table…" is settled, and this is where the ladder learns it.
 *
 * @param {{ status?: string, last_action_text?: string | null, status_basis_text?: string | null }} bill
 * @returns {boolean}
 */
export function isSettledFloor(bill) {
  if (bill?.status !== 'floor_vote') return false;
  const record = statusBasisText(bill);
  if (!FLOOR_SETTLED.test(record ?? '')) return false;
  // A dated calendar placement is a live fact whatever else its sentence says.
  return floorCalendarChamber(record) === null;
}

/**
 * HAS THE ANNOUNCING CHAMBER ALREADY ANSWERED — the sentence-level half of the
 * T0 settled guard, and the ONE place the record's passage vocabulary is read
 * as an OUTCOME rather than as a status.
 *
 * Returns the chamber whose floor answered the question in this sentence, or
 * null when the sentence records no outcome at all. Two non-chamber answers:
 *
 *   'both'     the measure is past both chambers ("Presented to President"),
 *              so whichever chamber announced it is done with it.
 *   'unknown'  an outcome the sentence does not attribute. "Motion to
 *              reconsider laid on the table Agreed to without objection." is
 *              written the same way in both chambers, and a defeat whose text
 *              names both chambers or neither reaches `floorSettledChamber`'s
 *              rule 7. Treated as an outcome, because the ladder's own
 *              discipline is that one missed crown is cheaper than one wrong
 *              one (see `floorPendingChamber`'s header in floor-text.mjs).
 *
 * THE RULE-RESOLUTION GUARD RUNS FIRST and it is not a nicety: "Rule H. Res.
 * 988 passed House." is the House agreeing to the TERMS of a debate that has
 * not happened yet, and reading it as a passage would retire the crown on the
 * exact day the bill reaches the floor — the opposite of this guard's purpose.
 * scripts/congress-fetch.mjs's `mapStatus` carries the same guard for the same
 * sentence, one layer down.
 *
 * A "received in" sentence is read as the OTHER chamber's outcome, because
 * that is what it records: the measure left the chamber that passed it and
 * arrived at the one that has not. This is why the guard is chamber-aware at
 * all — on 2026-09-17 sixty-odd bills on the House week-list read "Received in
 * the Senate", which answers the House's question and says nothing whatever
 * about the Senate's.
 *
 * @param {string | null | undefined} actionText
 * @returns {'house' | 'senate' | 'both' | 'unknown' | null}
 */
export function floorAnsweredChamber(actionText) {
  const t = String(actionText ?? '');
  if (!t) return null;
  if (/presented to president|became public law/i.test(t)) return 'both';
  // The rule, not the bill — see the header. Everything the passage rules
  // below would match in this sentence belongs to the resolution.
  if (!/\brule h\.? ?res\.? ?\d+ passed house/i.test(t)) {
    const passed = /\bpassed (?:the )?(house|senate)\b/i.exec(t);
    if (passed) return passed[1].toLowerCase();
    const agreed = /\b(?:passed\/)?agreed to in (house|senate)\b/i.exec(t);
    if (agreed) return agreed[1].toLowerCase();
    const received = /\breceived in (?:the )?(house|senate)\b/i.exec(t);
    if (received) return received[1].toLowerCase() === 'senate' ? 'house' : 'senate';
  }
  // A defeat, in the one vocabulary four readers share. A dated calendar
  // placement is a live fact whatever else its sentence says, so
  // `floorSettledChamber` excludes it and so does this.
  if (FLOOR_SETTLED.test(t) && floorCalendarChamber(t) === null) {
    return floorSettledChamber(t) ?? 'unknown';
  }
  // The post-passage motion, written identically in both chambers.
  if (/motion to reconsider laid on the table/i.test(t)) return 'unknown';
  return null;
}

/** A YYYY-MM-DD prefix, or null. Both sides of the date gate below are
 *  compared as strings, which is exact for this format and needs no clock.
...
/**
 * IS THIS ANNOUNCEMENT SPENT — the T0 half of the settled guard (owner
 * decision D13, 2026-09-18).
 *
 * THE MEASUREMENT THAT BOUGHT IT. The House week-list is a live T0 fact for
 * the dates it covers plus seven days, and it only drops a bill the chamber
 * itself removes — so a bill announced on Monday and passed on Tuesday kept
 * the crown all week. On 2026-09-03, 17 of the 18 bills the site was calling
 * "Deciding now" carried a floor outcome in their OWN record — fifteen
 * "Motion to reconsider laid on the table", one failed suspension vote, one
 * "Presented to President" — while the loudest surface on the site was still
 * saying "offices tally calls until it happens".
 *
 * THE SAME SHAPE AS THE T1 GUARD, ONE RUNG UP. `entersFloorWatch` refuses a
 * ripening-vote sentence whose own words say the question already resolved;
 * this refuses an ANNOUNCEMENT whose bill's record says the same thing. The
 * difference is that T0's evidence lives in two documents rather than one, so
 * the outcome has to be dated against the announcement to mean anything:
 *
 *   THE DATE GATE. The outcome must be dated on or after the day the
 *   announcement covers. Without it, a bill that passed the House in March and
 *   then appears on the SENATE's program in September would read as already
 *   answered — its record does carry a passage, and that passage is exactly
 *   why the Senate is taking it up. `covers` is the announcement's own horizon;
 *   an announcement that printed none falls back to its publication date, and
 *   one carrying neither cannot be dated against anything and is left alone.
 *
 *   THE CHAMBER GATE. The outcome must have happened in the chamber that did
 *   the announcing, or past both. See `floorAnsweredChamber`.
 *
 * WHAT THIS DOES NOT DO: it does not hide the bill, and it does not by itself
 * decide the rung. `docketRung` simply stops reading the announcement and lets
 * the bill's own record place it — which is how a bill the House passed on
 * Tuesday lands on T3 "Just passed a chamber" rather than being flattened into
 * the radar band (the K3 regression; see the T3 branch below).
 *
 * @param {{ last_action_text?: string | null, last_action_date?: string | null, status_basis_text?: string | null }} bill
 * @param {any} signal  data/floor-signals.json's entry for this bill
 * @returns {boolean}
 */
export function announcementAnswered(bill, signal) {
  const tier0 = signal?.tier0;
  if (!tier0) return false;
  const horizon = isoDay(tier0.covers) ?? isoDay(tier0.published);
  if (!horizon) return false;
  const acted = isoDay(bill?.last_action_date);
  if (!acted || acted < horizon) return false;
  const answered = floorAnsweredChamber(statusBasisText(bill));
  if (!answered) return false;
  if (answered === 'both' || answered === 'unknown') return true;
  return answered === tier0.chamber;
}
 ...
 * @param {number} [now]
 * @param {any} [checked]  data/floor-signals-checked.json, or null/undefined
 *   — merged into the freshness check via `floorSignalsHealthy` so a run that
 *   reconfirmed the schedule without any live BILL signal still counts.
 * @returns {'in_session' | 'out_of_session' | 'unknown'}
 */
export function chamberSessionFrom(meta, chamber, now = Date.now(), checked = null) {
 ...
 *
 * @param {{ status?: string, last_action_text?: string | null, last_action_date?: string | null, status_basis_text?: string | null }} bill
 * @param {any} signal
...
export function docketRung(bill, signal, ctx) {
  ...
  // The sentence the status was read from (statusBasisText) — the record's
  // latest step unless that step is one of the ambiguous notices. The DATE
  // stays the latest step's: freshness is about when the record last moved.
  ...
  /*
   * THE ANNOUNCEMENT, AND THE ONE THING THAT RETIRES IT (owner decision D13,
   * 2026-09-18). A live announcement still outranks whatever the action text
   * has decayed into — that is the whole of failure (2) in this file's header
   * — UNLESS the bill's own record says the announcing chamber has already
   * voted. `announcementAnswered` is that test, dated against the
   * announcement's own horizon and read in the announcing chamber.
   *
   * ANSWERED FALLS THROUGH; IT DOES NOT SHORT-CIRCUIT TO T4. The record then
   * places the bill exactly as it would for any un-announced measure, so a
   * defeat lands on T4 `just_decided` and a passage on T3 `just_passed` —
   * "Moving", with the honest label — instead of the whole of a busy House
   * week being flattened onto the radar band, which is the K3 regression the
   * T3 branch below exists to prevent. What the fall-through cannot produce is
   * T0: the announcement is spent, and no other rung can grant it.
   */
  const live = signalIsLive(signal, { now });
  const answered = live && announcementAnswered(bill, signal);
  if (live && !answered) {
    return {
      ...base,
      tier: 't0',
  ...
  /*
   * THE RESIDUAL RUNG, and the one place the spent announcement leaves a mark.
   * A bill whose announcement was answered but whose derived status carries no
   * rung of its own — "Motion to reconsider laid on the table Agreed to without
   * objection." derives `committee` until the next sync re-reads it — would
   * otherwise land here as an unlabelled radar bill, which is true but says
   * less than the record does. `just_decided` is the catalogue's existing "The
   * floor already answered" label; it is ink on a card and never colour.
   */
  return { ...base, tier: 't4', annotation: answered ? 'just_decided' : null };
}
