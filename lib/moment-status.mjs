...
export function billStatusLine(bill, now = Date.now()) {
  ...
  if (bill.status === 'passed_chamber' && !pending) {
    ...
    if (/^\s*Passed(?:\/agreed to in)? (?:House|Senate)\b/i.test(text)) {
      ...
      // goes to the President, and a concurrent resolution never does), but
      // the sentence itself says both chambers agreed to the same text — and
      // a concurrent resolution's path ends there.
      if (ps.stage === 'second' && concurrent && /\bwithout amendment\b/i.test(text)) {
        return line({ key: 'bothAgreed', terminal: true }, bill);
      }
