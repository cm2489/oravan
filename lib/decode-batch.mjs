 ...
 * rate this file exists to pay, plus — we believe, but have NOT verified
 * against an invoice — whatever the batch had already processed before the
 * cancel landed. And the fallback was not even
 ...
 * neither option gets the pre-2026-09-25 behaviour: one batch a round,
 * cancelled at the deadline, every stranded bill handed back for a synchronous
 * decode (tests/decode-batch.unit.spec.ts pins it unchanged).
 *
 ...
 *
 * `park` (2026-09-25) changes what giving up MEANS, and when. With it set, a
 * batch that runs out of wait (a failed status check no longer ends the wait
 * early — see the poll below), or ended but could not be read, is NOT
 * cancelled: it is still working (or already done) on requests
  ...
  while (current.processing_status !== 'ended') {
    if (now() >= deadline) {
      if (park) {
        return parkIt(
          pollErrors
            ? `still not seen to end after ${maxWaitMs}ms (${pollErrors} status check(s) failed along the way)`
            : `still processing after ${maxWaitMs}ms`
        );
      }
      log(
    ...
    //
    // A PARKABLE batch keeps polling instead (2026-09-25 fix pass). Giving up
    // on it early no longer costs a double payment — it costs every bill in it
    // a night's delay — and one transient 503 is not worth a night when the
    // batch may well end inside the wait it still has. So a failed status
    // check on a parkable batch is logged and the loop goes round again; the
    // deadline above is what parks it, exactly as it parks a slow one. The
    // time-critical batch (park false) keeps the cancel-and-fall-back-now
    // behaviour, because for its bills every minute of waiting is the cost.
    try {
      ...
      if (park) {
        ...
        if (pollErrors === 1) {
          log(
            `::warning::decode-batch: ${label} batch ${batch.id} could not be polled (${e.message}) — still inside its wait, so polling again; it is parked only if the wait runs out.`
          );
        }
        continue;
      }
 ...
 * work", and both must reach resolveCursorRows — which is why the result also
 * carries their union as `unresolvedSlugs` (see unresolvedSlugs below), the
 * one set the caller passes. A deferred (parked or held) bill is NOT a
 * failure — it never counts toward the honesty alarm (it is not in
 * `decodeAttempts` either; see the bump below) or the mostly-failed abort —
 * but the cursor must not walk past it any more than it may walk past a
 * failed one (#255's invariant: a queued decode that does not land never lets
 * the cursor advance past a missing bill).
 *
...
export async function drainDecodeQueue(decodeQueue, {
  ...
  if (queue.length === 0) {
    return {
      ...tally, failedSlugs, deferredSlugs, unresolvedSlugs: unresolvedSlugs({ failedSlugs, deferredSlugs }),
  ...
  for (const job of queue) {
    ...
    if (dec) {
      ...
      // path against ("reached the model N times, landed none"), so it counts
      // exactly the jobs whose verdict is DUE TONIGHT — one bump per job that
      // either lands or fails here, through the batch or the fallback.
      //
      // A PARKED JOB IS NOT BUMPED, and the order above is what makes that so
      // (2026-09-25 fix pass). It did reach the model, but its verdict is due
      // next run, and it comes back out of `added` without going into any
      // failure tally. Bumped, it would make `decodeAttempts > 0 &&
      // billsAdded === 0` true on a night where every bill parked, and then
      // one unrelated Congress.gov refresh failure (which `billsFailed` also
      // counts) would red a healthy run as "a dead decode path". Not bumped,
      // the rule keeps meaning what it meant before parking: every decode that
      // was due tonight was tried and none landed. A job held behind a running
      // batch (above) and one assembled entirely from a parked batch issue no
      // request tonight and are not bumped either.
...
/**
 * THE SET THE CURSOR FREEZES ON: every queued bill the drain did not land,
 * whether it failed or is waiting on a batch (parked tonight, or held behind
 * one an earlier run parked). #255's invariant — a queued decode that does not
 * land never lets the cursor advance past a missing bill — is kept by handing
 * THIS set, and nothing narrower, to scripts/sync-bills.mjs's
 * resolveCursorRows. It is a named function (2026-09-25 fix pass) rather than
 * a union spelled out at the call site so the call site has one thing to pass
 * and a test has one thing to pin: tests/decode-batch-park.unit.spec.ts checks
 * both what this returns and that sync-bills.mjs passes it.
 *
 * @param {{ failedSlugs?: Iterable<string>, deferredSlugs?: Iterable<string> }} drain
 * @returns {Set<string>}
 */
export function unresolvedSlugs({ failedSlugs = [], deferredSlugs = [] } = {}) {
  return new Set([...failedSlugs, ...deferredSlugs]);
}
