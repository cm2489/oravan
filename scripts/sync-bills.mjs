...
if (/(^|\/)sync-bills\.mjs$/.test(process.argv[1] ?? '')) {
  ...
  console.log(
    `re-decode on new text: ${refreshedThisRun.size} refreshed bill(s) seen, ${countHinted} with a grown text-version count, ${probed} probed (limit ${redecodeProbeLimit}${probeFailed ? `, ${probeFailed} probe failure(s)` : ''}), ${newTextCandidates.length} candidate(s) detected (${newTextCandidates.filter((c) => c.reason === 'new-text-version').length} newer than our stamp, ${newTextCandidates.filter((c) => c.reason === 'legacy-backfill').length} unstamped backfill), ${forceRedecodeSlugs.size} forced; ${plan.run.length} will be re-decoded (cap ${plan.cap}), ${plan.deferred.length} deferred to a later run, ${countStamped} record(s) stamped with a first text-version count`
  );
  if (!Number.isFinite(REDECODE_MAX_PER_NIGHT) || REDECODE_MAX_PER_NIGHT < 0) {
    console.log(
      `::warning::REDECODE_MAX_PER_NIGHT was not a usable number, so the built-in ceiling of ${plan.cap} was used instead. Check the workflow input.`
    );
  }
