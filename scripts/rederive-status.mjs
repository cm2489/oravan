...
import { mapStatus, slugOf, urgencyScore } from './congress-fetch.mjs';

export const MAX_CHANGE_FRACTION = 0.02;

/**
 * Every bill whose stored status disagrees with what the current matcher
 * reads from its stored last action. Pure: reads, never mutates.
 * @param {Array<Record<string, any>>} bills
 * @returns {Array<{ slug: string, from: string, to: string }>}
 */
export function planRederive(bills) {
  const changes = [];
...
function listLines(changes) {
  return changes.map((c) => `  ${c.slug}: ${c.from} -> ${c.to}`).join('\n');
}

/**
 * The whole pass over a pair of corpora, file-free so tests can drive it.
 * @returns {{ code: number, log: string[], changes: Array<{slug:string,from:string,to:string}>, wrote: { en: boolean, es: boolean } }}
 */
export function runRederive(bills, es, { dryRun = false } = {}) {
  const log = [];
  const changes = planRederive(bills);
  ...
  if (!verdict.ok) {
    ...
    return { code: 1, log, changes, wrote: { en: false, es: false } };
  }
  if (dryRun) {
    ...
    return { code: 0, log, changes, wrote: { en: false, es: false } };
  }
  ...
  return { code: 0, log, changes, wrote: { en: changes.length > 0, es: esTouched } };
}
...
if (/(^|\/)rederive-status\.mjs$/.test(process.argv[1] ?? '')) {
  ...
  const { code, log, wrote } = runRederive(bills, es, { dryRun });
  for (const line of log) (code ? console.error : console.log)(line);
  // Written together, as sync-bills.mjs's writeCorpus does; the ES file only
  // when an entry in it actually changed, so a no-op night leaves it
  // byte-identical.
