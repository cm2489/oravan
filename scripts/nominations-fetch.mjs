...
export function isCivilianNomination(item) {
  return item?.nominationType?.isCivilian === true;
}
...
export async function fetchNominationsSince(since, opts = {}) {
  ...
  for (;;) {
    ...
    rawSeen += raw.length;
    items.push(...raw.filter(isCivilianNomination));
    offset += pageSize;
 ...
 * @returns {object} a data/nominations.json record
 */
...
export function refreshNominationFields(existing, item) {
  const text = item.latestAction?.text ?? null;
  existing.last_action_date = item.latestAction?.actionDate ?? null;
