...
Two constraints exist because breaking them has already killed a build or a spec:
...
2. **Every callable bill link stays inside `section[aria-labelledby="top-actions"]`** — the ≤3-click funnel and the freshness specs both read that boundary. `FloorVotePanel` carries a bill link, so it lives inside that section. Note the section must be **full-width with the max-width wrapper inside it**, because the panel is full-bleed; see the primitive's doc comment.
