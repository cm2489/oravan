...
>
> **RULED 2026-08-15 (R1) — amber additionally requires the chamber to be MEETING.** The first recess this system lived through exposed the gap: hr-3633-119's Senate cloture motion of Aug 8 was true, dated, and inside the 14-day window — and the Senate had adjourned the next day, so the site's loudest "this is live, call now" surface shouted through a week in which no vote could occur. The owner's ruling: the two *record* facts (a calendar placement, a pending floor vote) may light amber and the green panel **only while the chamber the fact names is meeting.** When that chamber's own Daily Digest gives its next meeting as a pro forma session (`_meta.in_session`, read through `chamberSession` in `lib/docket.ts`), the homepage crown passes the candidate over — a full recess is a quiet week, which this file already sanctions — and the bill page prints a **ruled-paper note** (`components/FloorRecessNote.tsx`: hairline rules on paper, ink text, expressly *not* a new full-bleed ground) in the band's place: the Digest's own next-meeting sentence printed verbatim (`lang="en"`, ruling V4's pattern), its publication date, its URL, the checked-at stamp, and the fact that a bill cannot be called up at a pro forma session (glossary term #12). Four constraints, all enforced in code: **the status label is untouched** — a suspended band still *returns*, carrying a `suspended` flag, so "Floor vote pending" can never regress to "Floor activity" (the #207 seam, now pinned by test); **`announced` is exempt by construction** — a chamber that names a bill on its own published schedule is meeting, and the exemption lives in `floorFactSuspended`'s kind clause (`lib/journey.ts`), not in caller discipline; **`unknown` fails safe to amber** — a dead pipeline must never suppress a live claim, so a digest older than the 48-hour ceiling reads as in-session; and **the copy never says "recess" and never claims a duration** — the record carries one session verdict and one next-meeting label, so those are the only facts the sentence states. The window arithmetic is untouched (`SIGNAL_WINDOW_DAYS` stays 14; option "count session days" was parked for want of a forward calendar source). Self-heals when a chamber returns: the session verdict is part of the hourly write fingerprint (#230), so the flip forces a data commit and a rebuild. This ruling *narrows* amber; it changes nothing about which facts may be asserted (2026-08-09), which document may be quoted (V1), or which date is printed — that question stays open below. Enforced by `floorFactSuspended` + `billFloorBand` (`lib/journey.ts`) and `selectFloorVoteFeature` (`components/system/FloorVotePanel.tsx`); pinned by `tests/journey.unit.spec.ts` ("the session gate") and the session-aware skips in `tests/freshness.spec.ts`, whose status-line assertions deliberately run unskipped in both locales as the regression guard.
>
> **The printed-date ruling above is still open.** N3 changes what the label may *assert* about age; V1 adds a second document whose OWN dates may be printed and quoted; R1 gates when a record fact may light at all. None of the three settles which date the amber prints for a corpus fact, and `FloorVotePanel` still takes a caller-supplied `dateLabel` and still refuses to render without one.
...
>
> 1. **Only counted facts, and only ones already stored.** Outlet counts, their leans, the consecutive-week count, the rank — each is read straight from the committed file, and `scripts/check-conversation.mjs` fails the build if a domain with no AllSides rating is ever counted toward one. Nothing is inferred, rounded, or adjectival: no "widely", no "major", no "growing". *(Amended 2026-09-26, owner ruling: every outlet entry now stores the link to the story that made it count — `conversation/v2` — and the gate fails any entry seen after `_meta.links_since` without one, so "every count comes from stored evidence you can check" is checkable down to the article, not only the domain.)*
...
> 6. **No more than two of the six cards may owe their place to the most-viewed list**, by either admission route (two consecutive weeks, or one rated article beside a first-week listing). congress.gov's view counts are the cheapest input in this design for an outsider to move, so they may season the band and never fill it. *(Also amended 2026-08-12: the cap had been written against one of the two routes, and the other walked past it — five of six cards could have rested on a view count apiece.)* *(Amended 2026-09-26, owner ruling: those two slots go to the list's own top ranks, newest list first — before this they fell to alphabetical slug order, which kept H.R. 1 in the band 25 of 32 days while the rank-1 bill never appeared — and a bill that is already law takes no most-viewed slot; it renders only when the press corroborates it, as any two-outlet card does.)*
>
