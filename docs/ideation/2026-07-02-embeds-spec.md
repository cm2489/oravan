...
5. **Analytics = tenant-level aggregate impression counts only, from server-side, token-resolved page-load counting.** Decision between "nothing at all" and "CDN-log aggregates": **ship aggregates, minimally.** Paid tenants genuinely need a number to justify renewal ("your embed loaded 41,200 times in October"). **[Corrected 2026-07 (S20) — this paragraph originally sketched a `/e/{tenant}/reps` path-based tenant ID plus a daily Vercel-request-count job; that mechanism was never built. The actual shipped tenancy architecture (S18/S19) is query-string capability tokens, not path-based tenant IDs, so the counting mechanism follows the same shape, corrected here rather than left as stale doc drift:]** every paid-tier widget URL already carries the tenant's capability token as a query param (§3.2) — the action panel counts an impression on its own fully-authorized live-render branch (after the token, domain, and bill checks all pass); rep-lookup and bill-card gain an *optional* `token` param that, when it resolves to an active tenant, counts that page load too (a token that's absent, invalid, or revoked never changes rendering — only the counting silently no-ops). Each count is a `tenantId:day` INCR in the counters database (`lib/impressions.ts`), TTL 400 days, scheduled non-blocking via `after()` so it can never delay or affect a response, failing open (silent) on write and closed (`503`, never a guessed number) on read. No per-visitor anything exists anywhere — IP/UA/referer never enter the write path structurally, not just by discipline (`noteImpression`'s only parameter is an already-resolved `tenantId`). Free tier gets no stats (no token exists to count against — upgrade nudge). A tenant reads their own monthly-aggregated counts via `GET /api/tenant/impressions` (`X-Oravan-Key`-authenticated), disclosed in the response itself as best-effort and spoofable (F6) — unauthenticated public writes can be inflated by a forged request exactly as easily as the site's own `calls:total` counters can (KTD-4's own accepted-residual framing); daily bucketing is the mitigation, not a claim of fraud-proofness.
