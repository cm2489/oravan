 ...
 */
/** The member's role label key: senator, delegate (non-voting jurisdictions), or representative. */
export function repRoleKey(
  rep: Pick<Legislator, 'type' | 'state'>
): 'senator' | 'delegate' | 'representative' {
  return rep.type === 'sen'
    ? 'senator'
...
/** Party as the record states it, as plain text: translated for the three
 *  labels we carry, the record's own word otherwise. Never a color. */
export function usePartyLabel(rep: Pick<Legislator, 'party'>): string | null {
  const t = useTranslations('reps');
  return rep.party && ['Democrat', 'Republican', 'Independent'].includes(rep.party)
    ? t(`party.${rep.party as 'Democrat' | 'Republican' | 'Independent'}`)
    : rep.party;
}

/** The squared portrait frame, initials behind the photo. Decorative: the
 *  name is always printed beside it (see the header comment above). */
export function RepPortrait({ rep, large = false }: { rep: Legislator; large?: boolean }) {
  const initials = `${rep.first?.[0] ?? ''}${rep.last?.[0] ?? ''}`.toUpperCase();
  return (
    <span
      className={`relative flex shrink-0 items-center justify-center overflow-hidden rounded-stamp border-[1.5px] border-line-strong bg-wash ${
        large ? 'h-36 w-30' : 'h-22 w-18'
      }`}
    >
      {initials && (
        <span
          aria-hidden="true"
          className={`font-extrabold text-ink-2 ${large ? 'text-xl' : 'text-md'}`}
        >
          {initials}
        </span>
      )}
...
/**
 * The dial and the local numbers: the part of a rep card you press. Shared
