...
| `data/nominations.json` | Civilian Senate nominations (PNs) of the 119th Congress — Congress.gov's own citation, description sentence, and latest action, plus a status derived from that action text by `lib/nomination-status.mjs`'s rule table. **No AI touches this file**, and nothing in the app reads it yet: the data source ships ahead of any surface. Military promotion lists are excluded (no description, no nameable nominee). | `scripts/sync-nominations.mjs` (nightly, one free request; gated by `scripts/check-nominations.mjs`) |
